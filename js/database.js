...
window.DB = (function () {
  ...
  async function addActivityLog(message) {
    // Non-critical: never let a logging failure block the real action.
    try {
      unwrap(
        await client.from("activity_log").insert({ message: String(message) }),
        "Couldn't write to the activity log"
      );
    } catch (e) {
      console.warn(e);
    }
  ...
  /** Wipe contributions + activity log, un-release every payout, clear the PIN. */
